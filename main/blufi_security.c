...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    ...
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            LOG_ERROR("%s, blufi_sec->dh_param == NULL\n", __func__);
        ...
        if (ret) {
            LOG_ERROR("%s read param failed %d\n", __func__, ret);
        ...
        if (ret) {
            LOG_ERROR("%s make public failed %d\n", __func__, ret);
...
void loginfotest(void)
{
	LOG_INFO("test log info for thread test.");
}
